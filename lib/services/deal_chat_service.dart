...
class DealChatService {
  ...
  Stream<List<Map<String, dynamic>>> coRidersStream(String rideId) {
    ...
    return FirebaseFirestore.instance
        ...
        .snapshots()
        .map((snap) => snap.docs
            .where((d) => d.data()['customerId'] != uid)
            .map((d) {
              final data = d.data();
              final name = (data['customerName'] ?? 'Passenger').toString();
              final first = name.split(' ').first;
              final address = (data['passengerPickupAddress'] ?? '').toString();
              final area = address.split(',').map((p) => p.trim()).where((p) => p.isNotEmpty).toList();
              final pickupArea = area.length >= 2 ? area[area.length - 2] : (area.isNotEmpty ? area.first : 'Along route');
