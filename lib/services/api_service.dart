  ...
  static Future<Map<String, dynamic>> delete(String path) async {
    return _withRetry(() async {
      try {
        ...
        _checkStatus(response.statusCode, 'DELETE $path', response.body);
        return {};
      } catch (e) {
        if (e is ApiAuthException) rethrow;
        debugPrint('ApiService DELETE EXCEPTION: $e');
        rethrow;
      }
    });
  }

  /// Fire-and-forget health ping to verify the LAN backend during splash.
  static void wakeBackend() {
    final uri = Uri.parse(
      ApiConfig.baseUrl.replaceFirst('/api', '/health'),
    );
    http.get(uri).timeout(const Duration(seconds: 8)).ignore();
  }
}
