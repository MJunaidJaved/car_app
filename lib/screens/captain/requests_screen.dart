...
class _RequestsScreenState extends State<RequestsScreen>
            ...
            Row(
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _ride!.isShareRide
                        ? Colors.green[50]
                        : Colors.deepOrange[50],
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
                    _ride!.isShareRide ? 'SHARE RIDE' : 'SOLO RIDE',
                    style: TextStyle(
                      color: _ride!.isShareRide
                          ? Colors.green[800]
  ...
  Widget _tabLabel(String label, int count) {
    // A non-scrollable TabBar gives each of the 3 tabs a fixed, equal
    // width (screen width / 3). On narrower phones — or with a larger
    // system font size — "Confirmed" plus its red count badge doesn't
    // fit in that width and Flutter throws a RenderFlex overflow (the
    // "RIGHT OVERFLOWED BY x PIXELS" strip). Wrapping in a FittedBox lets
    // the label shrink to whatever width is actually available so it
    // never overflows, on any device or text-size setting.
    return FittedBox(
  ...
  Widget _buildConfirmedList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        return Container(
          key: ValueKey('confirmed_deal_${dealId.isNotEmpty ? dealId : i}'),
  ...
  Widget _buildList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        final dealId = d['id'] ?? '';
        final status = d['status'] ?? '';
        // ✅ Backward compatible: older deals created before this field
        // existed fall back to comparing against the ride's listed fare.
        final isNegotiated = d['isNegotiated'] is bool
            ? d['isNegotiated'] as bool
            : (fare != _ride!.suggestedFare);
        ...
        return Container(
          key: ValueKey('deal_${dealId.toString().isNotEmpty ? dealId : i}'),
          ...
          child: Column(
            ...
            children: [
              ...
              if (isPending) ...[
                ...
                // ✅ If the passenger accepted the ride's listed price
                // directly (didn't negotiate), the price is already fixed
                // — there's nothing to adjust, so only Accept is shown.
                if (isNegotiated)
                  Row(
                    children: [
                      Expanded(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(
                        'Accept · Fixed Price Rs ${fare.toStringAsFixed(0)}',
