...
class _FareNegotiateScreenState extends State<FareNegotiateScreen> {
  ...
  Future<void> _confirmAndBook() async {
    ...
    try {
      ...
      if (mounted) {
        // ✅ Single confirmation only. Previously this showed a SnackBar
        // AND a blocking "Got it!" dialog for the same event, forcing the
        // passenger to acknowledge the same booking twice before they could
        // move on. One clear SnackBar is enough — the booking details
        // (including the captain's contact / WhatsApp) are available right
        // away on the My Bookings screen.
        AppHelpers.showSnackBar(
          context,
          '✅ Booking confirmed! Rs ${_finalAgreedFare.toStringAsFixed(0)} - contact details are ready.',
        );
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: widget.ride.isShareRide
                              ? Colors.green.withValues(alpha: 0.25)
                              : Colors.deepOrange.withValues(alpha: 0.25),
                          borderRadius: BorderRadius.circular(16),
                        ),
                        child: Text(
                          widget.ride.isShareRide ? 'SHARE RIDE' : 'SOLO RIDE',
                          style: const TextStyle(
                            color: AppColors.white,
                            fontSize: 11,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ✅ Once the fare is settled, the passenger can already reach the
          // captain directly — no need to wait for a separate confirmation
          // step. Only shown to the passenger side (the captain already has
          // Call/WhatsApp actions on the Requests screen).
          if (!widget.isCaptain &&
              (widget.ride.captainPhone?.trim().isNotEmpty ?? false)) ...[
            const SizedBox(height: 10),
            Row(
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () =>
                        dialPhone(context, widget.ride.captainPhone),
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () =>
                        openWhatsApp(context, widget.ride.captainPhone),
