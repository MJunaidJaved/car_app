              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      const Column(
                        ...
                        children: [
                          Text('My Rides', style: TextStyle(color: AppColors.white, fontSize: 20, fontWeight: FontWeight.w800)),
                          Text('Manage your postings', style: TextStyle(color: Color(0xAAFFFFFF), fontSize: 13)),
                        ],
                      ),
                    ],
                  ),
                ...
                Padding(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: ['All', 'Office', 'Tour', 'Delivery'].map((filter) {
                        return Padding(
                          ...
                          child: FilterChip(
                            ...
                            labelStyle: const TextStyle(color: AppColors.bark, fontWeight: FontWeight.w700, fontSize: 12),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: AppColors.sage.withOpacity(0.2))),
                          ),
                        );
                      }).toList(),
                    ),
                    ...
                    children: [
                      Builder(builder: (context) {
                        ...
                        final activeRides = _rides.where((r) {
                          ...
                          return (r['rideType'] ?? '').toString().toLowerCase() == _typeFilter.toLowerCase();
                        }).toList();
                        if (activeRides.isEmpty) {
                          return Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.directions_car_outlined, color: AppColors.moss.withOpacity(0.3), size: 64),
                                const SizedBox(height: 12),
                                const Text('No rides posted yet. Post your first ride.', style: TextStyle(color: AppColors.sage, fontSize: 16, fontWeight: FontWeight.w600)),
                              ],
                            ),
                          );
                        }
                        return ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return _CaptainRideCard(
                              ...
                              suggestedFare: (r['suggestedFare'] ?? 0).toDouble(),
          ...
          Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: (rideType == 'tour' ? Colors.blue : AppColors.moss).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text(rideType.toUpperCase(), style: TextStyle(color: rideType == 'tour' ? Colors.blue : AppColors.moss, fontSize: 10, fontWeight: FontWeight.w800)),
              ),
              Text('${dt.day}/${dt.month} at ${dt.hour}:${dt.minute.toString().padLeft(2, '0')}', style: const TextStyle(color: AppColors.sage, fontSize: 12, fontWeight: FontWeight.w600)),
            ],
          ),
          const SizedBox(height: 16),
          Row(
            children: [
              const Icon(Icons.location_on_rounded, color: AppColors.moss, size: 16),
              const SizedBox(width: 8),
              Expanded(child: Text(startLocation, style: const TextStyle(color: AppColors.bark, fontSize: 15, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis)),
            ],
          ),
          const SizedBox(height: 4),
          Padding(
            padding: const EdgeInsets.only(left: 7),
            child: Container(width: 2, height: 12, color: AppColors.sage.withOpacity(0.2)),
          ),
          const SizedBox(height: 4),
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text(endLocation, style: const TextStyle(color: AppColors.bark, fontSize: 15, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis)),
            ],
          ),
          const SizedBox(height: 20),
          Row(
            children: [
              _InfoPill(icon: Icons.event_seat_rounded, label: '$availableSeats/$totalSeats seats'),
              const Spacer(),
              Text('Rs ${suggestedFare.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.bark, fontSize: 18, fontWeight: FontWeight.w900)),
            ],
          ),
          if (isActive) ...[
            const SizedBox(height: 20),
...
class _InfoPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.bg, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.sage.withOpacity(0.1))),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
