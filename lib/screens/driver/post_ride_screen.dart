...
class _PostRideScreenState extends State<PostRideScreen> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (args != null) {
        ...
        if (args.containsKey('ride')) {
          ...
          _editingRideId = (ride['id'] ?? '').toString();
          _fromCtrl.text = RideModel.formatLocationLabel(ride['startLocation']);
          _toCtrl.text = RideModel.formatLocationLabel(ride['endLocation']);
  ...
  Future<void> _openMapPicker({required bool isFrom}) async {
    final result = await Navigator.push<MapPickerResult>(
      ...
      MaterialPageRoute(
        builder: (_) => MapPickerScreen(
          title: isFrom ? 'Pick route start' : 'Pick route end',
          initialLat: isFrom
              ? (_startLat != 0 ? _startLat : null)
              : (_endLat != 0 ? _endLat : null),
          initialLng: isFrom
              ? (_startLng != 0 ? _startLng : null)
              : (_endLng != 0 ? _endLng : null),
        ),
      ),
    );
    ...
    setState(() {
      if (isFrom) {
        _startLat = result.lat;
        _startLng = result.lng;
  ...
  Future<void> _post() async {
    ...
    try {
        ...
        if (mounted) {
          await showDialog<void>(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text(
                'Complete your captain profile (phone, city, vehicle details, and car photo) before posting rides.',
              ),
              actions: [
                TextButton(
                    onPressed: () => Navigator.pop(ctx),
      ...
      if (_editingRideId != null && _editingRideId!.isNotEmpty) {
        ...
        if (mounted) {
          AppHelpers.showSnackBar(context, 'Ride updated successfully!');
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const Text('Fill your empty seats',
                                  style: TextStyle(
                              ...
                              children: [
                                ...
                                Align(
                                  ...
                                  child: TextButton.icon(
                                    onPressed: () =>
                                        _openMapPicker(isFrom: false),
                                    icon: const Icon(Icons.map_outlined,
                                        size: 18),
                                    label: const Text('Pick drop on map'),
                                  ),
                                ),
                                const SizedBox(height: 12),
