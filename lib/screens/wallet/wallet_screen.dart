...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> _loadWallet() async {
    try {
      ...
      setState(() {
        ...
        _transactions = List<Map<String, dynamic>>.from(txRes['transactions'] ?? []);
        _loading = false;
      });
    } catch (e) {
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              ...
                              style: TextStyle(color: AppColors.bark, fontSize: 12, height: 1.3),
                            ),
                          ),
                        ],
                      ),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
  _loading ? '...' : 'Rs ${_balance.toStringAsFixed(0)}',
  ...
  builder: (context) => _TopUpSheet(onTopUp: _topUp),
);
                                },
                              ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  onTap: () {
                                    Clipboard.setData(const ClipboardData(text: 'POOL2024'));
                                    ScaffoldMessenger.of(context).showSnackBar(
  ...
  const Padding(
    ...
    child: Text('No transactions yet.', style: TextStyle(color: AppColors.sage)),
  )
else
  ..._transactions.map((tx) {
    final isDebit = tx['type'] == 'debit' || tx['type'] == 'commission';
    final amount = (tx['amount'] ?? 0).toDouble();
    final date = tx['createdAt'] ?? '';
    return _TransactionItem(
      title: tx['description'] ?? tx['type'] ?? 'Transaction',
      date: date.length > 10 ? date.substring(0, 10) : date,
      amount: '${isDebit ? '-' : '+'}Rs ${amount.toStringAsFixed(0)}',
      isDebit: isDebit,
    );
  }),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const Text('Top Up Wallet', style: TextStyle(color: AppColors.bark, fontSize: 22, fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          const Text('Choose an amount to add', style: TextStyle(color: AppColors.sage, fontSize: 14)),
          const SizedBox(height: 24),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [500, 1000, 2000].map((amt) => GestureDetector(
              onTap: () {
              ...
              child: Container(
                ...
                child: Text('Rs $amt', style: const TextStyle(color: AppColors.bark, fontWeight: FontWeight.w700)),
              ),
            )).toList(),
          ),
          const SizedBox(height: 24),
