...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    // Car photo required only if not already uploaded
    final existingPhoto = (_userData?['vehiclePhotoUrl'] ?? '').toString().trim();
    if (existingPhoto.isEmpty && _vehiclePhoto == null) {
    ...
    setState(() => _isLoading = true);
    try {
      // Upload car photo only if a new one is selected
  ...
  Widget build(BuildContext context) {
    final existingPhoto = (_userData?['vehiclePhotoUrl'] ?? '').toString().trim();
    final carPhotoAlreadyUploaded = existingPhoto.isNotEmpty;
              ...
              child: ListView(
                ...
                children: [
                  Container(
                    ...
                    child: const Text(
                      'Keep your captain profile complete so passengers can trust you.',
                      style: TextStyle(
                        color: AppColors.deepNavy,
                  ...
                  // Car photo: show upload only if not already uploaded
                  if (!carPhotoAlreadyUploaded) ...[
                    ...
                    _UploadTile(
                      ...
                      existingUrl: null,
                      onTap: () =>
                          _pickImage(onPicked: (f) => setState(() => _vehiclePhoto = f)),
                    ),
                    const SizedBox(height: 20),
                  ] else ...[
                    // Show existing car photo (read-only tap to change)
                    _sectionTitle('Car Photo'),
                    _UploadTile(
                      label: 'Car Photo',
                      image: _vehiclePhoto,
                  ...
                  TextFormField(
                    ...
                    decoration: _inputDecoration('Car Name/Model (e.g. Corolla)'),
                    validator: _required,
                  ),
                  const SizedBox(height: 10),
                  TextFormField(
